        ...
        PremiumCard {
            VStack(alignment: .leading, spacing: 12) {
                ...
                Button { showRecoveryPIN = true } label: {
                    Label(model.hasRecoveryPIN ? "Modifica PIN di recupero" : "Imposta PIN di recupero", systemImage: "number.square.fill").frame(maxWidth: .infinity, alignment: .leading)
                }
                .buttonStyle(.bordered)
                Text("Il PIN di 6 cifre consente di reimpostare la password senza server o database.").font(.caption).foregroundStyle(.secondary)
            }
        }
    }
        ...
        NavigationStack {
            Form {
                Section("PIN di recupero") {
                    ...
                    SecureField("Ripeti il PIN", text: $confirmation).keyboardType(.numberPad)
                        .onChange(of: confirmation) { _, value in confirmation = String(value.filter(\.isNumber).prefix(6)) }
                }
                Button("Salva PIN") {
                    if model.setRecoveryPIN(pin) { dismiss() }
                }.disabled(pin.count != 6 || pin != confirmation)
            }
            .navigationTitle(model.hasRecoveryPIN ? "Modifica PIN" : "Imposta PIN")
            .navigationBarTitleDisplayMode(.inline)
            .toolbar { ToolbarItem(placement: .cancellationAction) { Button("Chiudi") { dismiss() } } }
        }
    }
